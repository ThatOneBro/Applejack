--[[
	~ Serverside Config ~
	~ Applejack ~
--]]

--Remove the commented out lines
GM.Config["MySQL Host"] = "localhost"; -- The host that your MySQL database is located.
GM.Config["MySQL Username"] = "root"; -- The username that you log into MySQL with.
GM.Config["MySQL Password"] = "blabla123"; -- The password that you log into MySQL with.
GM.Config["MySQL Database"] = "applejack"; -- The name of the database that we'll be using.
GM.Config["MySQL Port"] = 8888; -- The port for the MySQL database; Default is 3306
GM.Config["MySQL Table"] = "players"; -- The name of the table where the player data is stored.